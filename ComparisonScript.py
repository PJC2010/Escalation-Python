...
class AdherenceComparer:
   ...
   def process_weekly_data(self, folder_path, date):
       ...
       for file_name in worklist_files:
           ...
           try:
               ...
               available_columns = [col for col in key_columns if col in column_mapping.keys()]
   ...
   def compare_weeks(self):
       ...
       previous_data = self.process_weekly_data(previous_folder, self.previous_date)
       ...
       for market_code in set(list(current_data.keys()) + list(previous_data.keys())):
           try:
               ...
               filename = f"Week of {self.current_date} {market_code} Escalations Comparison.xlsx"
               file_path = self.output_folder / filename
               with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                   # Weekly Data Tabs
                   if not curr_df.empty:
                       curr_df.to_excel(writer, sheet_name='Current Week', index=False)
                   if not prev_df.empty:
                       prev_df.to_excel(writer, sheet_name='Previous Week', index=False)
                   # New Members Tab (in current but not in previous)
                   if not curr_df.empty and not prev_df.empty:
                       new_members = curr_df[~curr_df['PayerMemberId'].isin(prev_df['PayerMemberId'])]
                       if not new_members.empty:
                           new_members.to_excel(writer, sheet_name='New Members', index=False)
                   # Previous Week Only Tab (in previous but not in current)
                   if not curr_df.empty and not prev_df.empty:
                       resolved = prev_df[~prev_df['PayerMemberId'].isin(curr_df['PayerMemberId'])]
                       if not resolved.empty:
                           resolved.to_excel(writer, sheet_name='Previous Week Only', index=False)
                   # Weekly Summary Tab
                   summary_data = {
                       'Metric': [
                       ...
                       'Value': [
                           ...
                           (len(curr_df) if not curr_df.empty else 0) -
                           (len(prev_df) if not prev_df.empty else 0),
                           curr_df['Practice Name'].nunique() if not curr_df.empty else 0,
                           prev_df['Practice Name'].nunique() if not prev_df.empty else 0,
                           datetime.now().strftime('%Y-%m-%d %H:%M')
                       ]
                   }
                   pd.DataFrame(summary_data).to_excel(writer, sheet_name='Weekly Summary', index=False)
